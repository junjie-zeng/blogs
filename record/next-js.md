...
对于单一的页面会出现两个不友好的问题：
1. 首先，因为就一个页面，所以需要将所有的资源加载到这个页面当中，就会出现首屏加载过慢（剩下的加载正常）。
2. 不能seo(搜索引擎优化)，比如一个博客、门户网站对于单页面来说是不太适合的。

所以有了next.js，它是一个基于React的框架，可以进行服务端渲染，也就是在服务端做将我们请求的页面拼接好，首先它解决了首屏加载变得更加迅速，其次优化了seo.

接着我说一下它的优点：
1. 搭建便捷（因为它是基于react的，所以有一个完善的架构，搭建起来非常轻松，内部都已经配置好了，不需要额外配置，我们在使用时减少了很多工作量。）
...
4. 灵活的配置（根据配置项的修改，可以快速生成我们想要的样子、比如：ssg）
