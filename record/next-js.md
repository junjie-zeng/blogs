...
什么是服务端渲染
什么是客户端渲染
我们平常写的vue、react大部分都是单页面应用，开发后实际上就是一个页面
对于单一的页面会出现两个不好的问题：
1. 因为就一个页面，所以需要将所有的资源加载到这个页面当中，就会出现首屏加载过慢（剩下的加载正常）。
2. 不能seo(搜索引擎优化)，比如一个博客、门户网站对于单页面来说是不太适合的。

所以有了next.js，它可以进行服务端渲染，也就是在服务端做好这些事情，首先它解决了首屏加载变得更加迅速，其次优化了seo.

接着我说一下它的优点：
1. 搭建轻松（首先它是基于react的，所以有一个完善的架构，搭建起来非常轻松，内部都已经配置好了，不需要额外配置，给我们减少了很多工作量。）
2. 自带数据同步（我们做这个ssr最大难点就是服务端把数据渲染好了以后，拿到客户端要重新渲染也就是随时变化服务端那么我客户端也要发生变化，这个在没有框架之前是非常复杂的，有了next.js之后它给我们提供了非常好的解决方法，让我们轻松的实现这些步骤，什么都不用做就帮我们实现了前后端自动数据同步）
3. 丰富的插件（自己形成了生态，减少了造轮子的步骤）
4. 灵活的配置（根据配置项的修改，可以快速生成我们想要的样子）
...
2. next.js的优势 ， 解决了什么问题
  - 服务器端渲染（SSR）
    问题：React、Vue 应用默认是客户端渲染，这意味着整个页面在客户端生成。(最常见的现象，请求一个url得到的不是一个完整的页面，页面中的内容是由js发起异步请求，然后拿到数据再做页面渲染)
    解决方案：使用服务器端渲染，页面在服务器上预先生成 HTML，然后发送到客户端。
      优点：(页面可以更快的呈现出来，搜索引擎可以更容易的抓取页面内容)
